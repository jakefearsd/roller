...
The above instructions rely on Roller’s default (and only supported)
user authentication mechanism: a Roller-provided database table
(roller_user) storing usernames and encrypted passwords. Earlier
versions of Roller also supported OpenID, an OpenID/DB combination, and
LDAP authentication; those methods have been removed, and
"authentication.method" in roller.properties now only accepts the value
_db_.
...
out of Roller, including how to setup Roller to use server-provided
resources.
...
done), you must migrate your old file uploads to the new Media Blogging
system yourself; Roller no longer does this automatically on startup.
Ensure the two properties below are set correctly before you start
Roller 5.0/5.1 for the first time, then copy the files from _uploads.dir_
into _mediafiles.storage.dir_ by hand.
