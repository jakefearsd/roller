...
category link, feed links, and feed auto-discovery. Check the
  ...
  <table border="0" align="center" width="95%">
    <tr>
      ...
      <td width="20%" valign="top">
        <h2>Feeds</h2>
        #showAtomFeedsList($model.weblog) #10
        <h2>Search</h2>
        #showWeblogSearchForm($model.weblog false) #11
        <h2>Links</h2>
        #set($defaultFolder = $model.weblog.getBookmarkFolder("/")) #12
        #showBookmarkLinksList($defaultFolder)
        <h2>Navigation</h2>
        #showPageMenu($model.weblog) #13
        #showAuthorMenu(true) #14
        <h2>Referrers</h2>
        #set($refs = $model.weblog.getTodaysReferrers()) #15
...
the HTML _<link>_ elements required for Atom feed auto-discovery.
...
your weblog. There are macros for displaying your weblog entries and
...
search form macro like so:

*#showWeblogSearchForm($model.weblog false)*

Argument one is the weblog to search and argument two is whether to show
a category chooser alongside the search box. Note that the arguments for
a macro are separated by a space and NOT a comma as was the case for
methods.
...
|$config.feedMaxSize
|Integer
|Maximum number of items displayed in Atom feeds.
...
Show the Atom auto-discovery links suitable for use within
...
Adds the Umami analytics tracking script for this weblog, built from its
validated analytics site id (configured on Weblog Settings). Emits
nothing when the weblog has no analytics site id configured — there is
no server-level default to fall back to. This tag is normally placed
within the HTML header section.
...
There is an additional model that can be made available to Roller
weblogs by a site administrator: *$site*, for accessing site-wide data.

=== $site

The *$site* model provides access to site-wide data: aggregations of
webog entries from all weblogs, lists of
...
For some SiteModel methods (e.g. hot-blogs, etc.) return
