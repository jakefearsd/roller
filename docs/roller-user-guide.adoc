...
Readers can comment on your weblog entries. Comments are enabled by
default; you can turn them off, for the whole weblog or for one entry, on
the Weblog Settings page.
...
=== Keeping spam off your weblog

**Read this before you turn comments on.** Roller ships with no spam
filtering. There is no word list, no link or length check, and no
third-party service such as Akismet — earlier versions had some of these
and they have been removed. The comment authenticator that ships by
default accepts every commenter without a challenge. So an open comment
form on a public weblog will collect spam, and nothing will stop it
arriving.

What Roller does give you is control over what gets *published*:

* *Comment moderation*. The one measure that actually works. With
moderation on, a new comment is held as pending and appears on your
weblog only after you approve it on the Comments page. If your weblog is
public and you are not watching it closely, leave this on.
* *Email notification*. Notification of each new comment, so moderation
does not depend on you remembering to look. Both are on the Weblog
Settings page.
* *Marking spam*. Anything that does get through can be marked as spam
from the Comments page, which hides it without deleting it.
* *Comment throttling*. Your site administrator can limit how many
comments one address may post in a given period. It is **off by
default** and is configured in `roller.properties`
(`comment.throttle.*`), not in the web interface.
* *An IP ban list*. Site administrators can maintain a file of banned
addresses, re-read automatically when it changes.
* *Disallowing HTML in comments*. A site-wide setting. Comment text is
sanitized regardless, but turning HTML off entirely is stricter.

If you need real spam filtering, the honest options today are to keep
comments moderated, or to turn comments off and invite replies by email
instead.
