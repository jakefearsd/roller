...
* *Weblog*: A set of web pages and Atom feeds that display weblog
...
weblog entries posted. Roller publishes feeds in the Atom format.
...
as a p__odcast__ in your weblog’s Atom feed, then enter the URL of that
file here.
...
your blog’s Atom feed, it will include the podcast as a feed
enclosure (https://en.wikipedia.org/wiki/RSS_enclosure) to make it easy
...
Advanced Settings section if you wish to add it to your blog’s Atom
feed.
...
* Roller will add the podcast to your Atom newsfeed as an _<enclosure>_.
You can check this by looking at your Atom newsfeed and any podcast
...
be stripped out in your Atom feed. You can access your title in a page
template with the expression _#showWebsiteTitle()_

* *Tagline* – Short description or sub title of your weblog. May include
HTML, but the HTML will be stripped out in your Atom feed. You can access
...
* *Locale* – sets the default locale for your weblog.
* *Timezone* – the timezone to be used in your weblog. This is also what
`Pub Time` on the entry editor is interpreted against when you set a
publication time.
...
Roller ships first-party analytics through
https://umami.is[Umami], served from your own blog’s domain. There is no
free-text tracking-code field to paste anything into – you connect your
weblog to a website Umami already knows about with two settings:

* *Analytics website ID* – the Website ID Umami assigns your site (find
it in Umami under *Settings → Websites → your site*). Leave this blank
to turn analytics off for this weblog.
* *Analytics share URL* – optional. Umami’s public share link for this
site, shown as a convenience link on this settings page.

Once a website ID is set, every bundled theme’s `<head>` automatically
emits the tracking script for you – there is no macro to add to a
template yourself.

=== Weblog maintenance operations

Maintenance operations moved out of the per-weblog Preferences tab and
into *Global Admin → Maintenance*, since these are operator actions
("the search index is corrupt", "the page cache is stale") rather than
day-to-day authoring – you’ll need a Global Administrator account to
reach the page. Choose the weblog to operate on from the dropdown first,
then use the buttons below. You’ll rarely need them, but they are handy
after upgrades or when something looks stale:

* *Flush Cache* – clears the chosen weblog’s page cache so that its
next page view is rendered fresh.
* *Rebuild Search Index* – schedules a rebuild of the search index for
the weblog’s entries.
...
image uploaded to the weblog. Use this once after upgrading to a
...
group blogging is the same as creating a personal weblog. Navigate to
the *Members* menu item in the *Preferences* tab. The Members page
enables weblog admins to add members to a group blog and manage the
group blog’s user access. Roller grants you Admin rights automatically
on any weblog you create.

image::user-guide-21-member.png[]

Before adding someone, it helps to understand the three permission
levels allowed for members of a weblog. They are:
...
users of the blog. Admins can see both the Create & Edit tab and the
...
Adding a member is immediate – there is no invitation, no notification
email, and nothing for the other person to accept. Near the top of the
Members page, type the username of an existing Roller account into the
*Username* field, choose their permission (Admin, Author, or Limited)
from the dropdown, and submit. They have access to the weblog as soon as
you submit the form; the next time they log in, the weblog shows up in
their Main Menu.

Once someone is a member of your blog, you can change their permission
or remove them from the table further down the same page: click the
radio button for their new permission (or the remove option) in their
row, then click *Save*. You cannot reduce your own permissions or remove
yourself from the weblog – ask another admin to do that for you.
...
(Atom) and on the default front page of the Roller.
...
newsfeeds (Atom) and on the default front page of the site.
* *Site Administrator’s emailConfiguring Roller Address*: admin’s email address, to be
include in side-side newsfeeds (Atom)
...
like the frontpage weblog’s Atom feed to be an aggregation of
...
in each newsfeed (Atom).
* *Display styled newsfeeds for browsers*: Set to true to enable
user-friendly Atom feed display, so that users don’t see raw XML
